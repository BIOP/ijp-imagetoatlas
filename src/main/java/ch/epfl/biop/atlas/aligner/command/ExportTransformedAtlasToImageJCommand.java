...
@Plugin(type = Command.class,
        menuPath = "Plugins>BIOP>Atlas>Multi Image To Atlas>Export>ABBA - Export Atlas Channel in Original Slices Coordinates to ImageJ",
        description =
...
public class ExportTransformedAtlasToImageJCommand implements Command {
    ...
    public void run() {
        ...
        IntStream.range(0,slicesToExport.size()).parallel().forEach(i -> {
            ...
            if (success) {
                ...
                mp.infoMessageForUser.accept("Export deformation field to ImagePlus","Slice "+slice+" done ("+(i+1)+"/"+images.length+")");
                images[i].show();
            } else {
